...
public class GenerateWorld : MonoBehaviour
{
    ...
    [SerializeField] private GameObject interactable_prefab;
    private Grid world;
    ...
    private void FillGround(Vector3Int minBounds, Vector3Int maxBounds, Tilemap ground, Tilemap ground_cosmetic, List<Tilemap> segments)
    {
        for(int x = minBounds.x; x < maxBounds.x; x++)
        {
            for(int y = minBounds.y; y < maxBounds.y; y++)
            {
                ...
                var random = Random.Range(0, floor_tile.Length);
                if (!IsWall(segments, pos))
                {
                    ground.SetTile(pos, floor_tile[random]);
                }
                else
                {
                    ground_cosmetic.SetTile(pos, floor_tile[random]);
                }
            }
        }
    }
    ...
    private Tilemap FixWalls(Tilemap segment_prefab, (Vector3Int point, DiagonalDirections self_corner) bounds)
    {
      ...
      /*   if(bounds.self_corner == DiagonalDirections.Zero) return segment; */
        ...
        var tilemapBounds = segment.cellBounds;
        ...
        while (true)
        {
            ...
            var cell = segment.WorldToCell(randomPosition);

            if (!segment.HasTile(cell))
                break;
        }
    
        Instantiate(interactable_prefab, segment.WorldToCell(randomPosition), Quaternion.identity);
