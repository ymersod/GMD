...
public class Spawner : MonoBehaviour
{
   ...
   public void SpawnQuickCastIndicator(GameObject caller, Vector2 start, Vector2 end, float duration, GameObject prefab, float length, float width)
   {
        ...
        indicator.transform.SetParent(caller.transform);

        StartCoroutine(MoveIndicator(caller, indicator_sprite_mask.transform.localPosition, maskLocalPosition, duration, indicator_sprite_mask));
   }

   private IEnumerator MoveIndicator(GameObject caller, Vector3 start, Vector3 end, float duration, GameObject indicator_mask)
   {
        float elapsedTime = 0;
        while (elapsedTime < duration && caller != null)
        {
